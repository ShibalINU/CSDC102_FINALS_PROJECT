...
/* 
 * checkAlligator(NodePtr head, int playerX, int playerY)
 * 
 * Returns true if the player is standing on a fake alligator log. 
 */ 
bool checkAlligator(NodePtr head, int playerX, int playerY)
{
    if (zoneMap[playerY] != ZONE_RIVER)
        return false;
...
void showTitleScreen()
{
  ...
  HOW TO PLAY:
    ...
    Avoid gators  (AAAA)  — fake logs in GREEN!
             ...
             int gameSpeed, int numTrucks, int numLogs)
{
    ...
    while (lives > 0 && crossings < WIN_CROSSINGS && !quitFlag)
    {
        ...
        // 6.5 Checks fake alligator logs
        else if (checkAlligator(head, playerX, playerY))
        {
            lives--;
            playerX = 20;
            playerY = TOTAL_ROWS - 1;
